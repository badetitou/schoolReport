...
Nous avons réussi à construire des bases solides pour améliorer l'outil de migration que nous avons commencé.
...
Afin d'améliorer la représentation que nous avons d'une application graphique,
    il faudrait que l'on représenté toutes les couches d'une interface graphique comment décrit Section \ref{guiDecomposition}.
Une fois le layout implémenté nous aurons bien représenter l'interface utilisateur, il nous faudra encore définir et implémenter
...
Entre autres, il est possible de configurer l'outil pour que l'application cible utilise tel ou tel autre framework
...
La migration d'un framework consiste en la détection des différents widgets qu'il définit,
    avec leurs compositions et leurs styles.
